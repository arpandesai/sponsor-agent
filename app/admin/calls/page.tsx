      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-muted)]">
            Every request made to a provider, in order, with its latency and cost.
          </p>
        </div>
          ...
          <tbody>
            {logs.map((row) => (
              <tr key={row.id} className="border-b border-[var(--color-border)]">
                ...
                <td className={`py-2 ${row.status === 'error' ? 'text-[var(--color-danger-ink)]' : ''}`}>
                  {row.status}
                </td>
                <td className={`py-2 ${row.latencyMs > 4000 ? 'text-[var(--color-danger-ink)]' : ''}`}>
                  {row.latencyMs}ms
                </td>
